...
bound=symmetrise(bound,'antipodal');% For Crystal 1
boundr=orib*bound;% For Crystal 2
% Rotate both into required position
bounda=ori*bound;
boundr=ori*boundr;
% Define the shear Directions
sd=Miller(0,bound.hkl(3),-bound.hkl(2),cs,'uvw');
sda=ori*sd;

%% Plot figure
f=figure(2);
clf
% Add poles for crystal 1
...
h5=plot(bounda,'upper','Marker','s','MarkerFaceColor',[0 0.4470 0.7410],'label',...
    ['\{',sprintf('%0.0f%0.0f%0.0f',bound.h(1),bound.k(1),bound.l(1)),'\}'],'textBelowMarker',...
    'DisplayName',['\{',sprintf('%0.0f%0.0f%0.0f',bound.h(1),bound.k(1),bound.l(1)),'\} Crystal 1']);
% Add poles for Crystal 2
...
h6=plot(boundr,'upper','Marker','s','MarkerFaceColor',[0.6350 0.0780 0.1840],'label',...
    ['\{',sprintf('%0.0f%0.0f%0.0f',bound.h(1),bound.k(1),bound.l(1)),'\}'],'textBelowMarker',...
    'textAboveMarker','DisplayName',['\{',sprintf('%0.0f%0.0f%0.0f',bound.h(1),bound.k(1),bound.l(1)),'\} Crystal 2']);

% Add loading geometry
h8=plot(cd1,'upper','Marker','+','MarkerEdgeColor',[0.9290 0.6940 0.1250],'LineWidth',2,...
    'label','CD1','textAboveMarker','DisplayName','Comp. Dir. 1');
h7=plot(sda,'upper','antipodal','Marker','x','MarkerEdgeColor',...
    [0.9290 0.6940 0.1250],'LineWidth',2,'label','SD1','textAboveMarker', ...
